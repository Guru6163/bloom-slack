...
import { getAppBaseUrl, getSlackOAuthRedirectUri } from './app-url';
...
export async function handleSlackOAuth(requestUrl: string): Promise<Response> {
  ...
  try {
    ...
    const { error: upsertError } = await supabase.from('workspace_configs').upsert({
      team_id: teamId,
      team_name: teamName,
      bot_token: botToken,
    ...
    if (upsertError) {
      console.error('[slack-oauth] workspace_configs upsert failed:', upsertError);
      throw new Error(
        `Database save failed: ${upsertError.message}. Check SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY and that table workspace_configs exists.`,
      );
    }
    console.info(`[slack-oauth] saved workspace_configs for team_id=${teamId} (${teamName})`);

    const baseUrl = getAppBaseUrl();
    const setupUrl = `${baseUrl}/api/slack/setup?token=${setupToken}`;
