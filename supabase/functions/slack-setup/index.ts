...
async function fetchBrandsForApiKey(apiKey: string): Promise<{ valid: boolean; brands: unknown[] }> {
  try {
...
serve(async (req: Request) => {
  ...
  if (method === 'POST' && url.pathname.endsWith('/validate-key')) {
  ...
  const isChangingBrandOnly = Boolean(ws.setup_completed && ws.bloom_api_key);
  const supabaseUrl = Deno.env.get('SUPABASE_URL')!;

  return new Response(buildSetupHTML(token, ws, isChangingBrandOnly, supabaseUrl), {
...
<body>
  ...
  <script>
    const SUPABASE_URL = '${escapeAttr(supabaseUrl)}';
    ...
    async function validateKey() {
      ...
      const res = await fetch(SUPABASE_URL + '/functions/v1/slack-setup/validate-key', {
        method: 'POST',
    ...
    async function loadBrandsWithToken() {
      const res = await fetch(SUPABASE_URL + '/functions/v1/slack-setup/brands', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    async function saveBrandCommon() {
      ...
      const res = await fetch(SUPABASE_URL + '/functions/v1/slack-setup/save', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          token: TOKEN,
